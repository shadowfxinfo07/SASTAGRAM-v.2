...
publishPostBtn.addEventListener('click', async () => {
    ...
    const postPicFile = document.getElementById('input-post-pic').files[0];

    if (!postPicFile) return alert("Please select a photo to share!");

    publishPostBtn.textContent = "Uploading...";
    publishPostBtn.disabled = true;

    const postImageUrl = await uploadToCloudinary(postPicFile);
    if (!postImageUrl) {
        alert("Failed to host post media image.");
    ...
    onSnapshot(feedQuery, (snapshot) => {
        ...
        snapshot.forEach((postDoc) => {
            ...
            card.innerHTML = `
                ...
                <div class="post-main-image" style="background: url('${post.postImage}') center/cover"></div>
